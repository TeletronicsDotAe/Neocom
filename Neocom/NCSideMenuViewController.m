...
- (void)setMenuViewController:(UIViewController *)menuViewController {
    ...
    if (_menuOnTop) {
        menuViewController.view.layer.zPosition = 1.0f;
        menuViewController.view.layer.shadowOpacity = 0.5f;
        menuViewController.view.layer.shadowColor = [[UIColor blackColor] CGColor];
        menuViewController.view.layer.shadowRadius = 4.0;
        menuViewController.view.layer.shadowOffset = CGSizeMake(0, 0);
        menuViewController.view.layer.shadowPath = [[UIBezierPath bezierPathWithRect:menuViewController.view.bounds] CGPath];
    }
...
- (void)setMenuVisible:(BOOL)menuVisible {
    ...
    self.menuViewController.view.transform = [self menuViewTransform];
...
- (void)setMenuVisible:(BOOL)menuVisible animated:(BOOL)animated {
    ...
    void (^willAppear)() = ^{
        ...
        if (menuVisible && !self.menuViewController.view.superview) {
            ...
            CGRect frame = self.view.bounds;
            frame.size.width -= NCSideMenuViewControllermMenuEdgeInset;
            // If the menu is on top, we scroll it out of view
            frame.origin.x -= _menuOnTop ? frame.size.width : 0;
...
- (CGAffineTransform)menuViewTransform {
    if (self.menuVisible) {
        return CGAffineTransformMakeTranslation(self.view.frame.size.width - NCSideMenuViewControllermMenuEdgeInset, 0.0f);
    }
    else
